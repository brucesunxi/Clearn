  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
      <div className="mb-8">
        ...
        <p className="text-gray-400 text-sm">{locale === 'zh' ? '选择文章开始口语练习' : 'Pick an article to practice speaking'}</p>
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
          {displayArticles.slice(artPage * 4, (artPage + 1) * 4).map((article) => (
            <button key={article.id} onClick={() => setStartedArticle(article)}
              className="group text-left bg-white rounded-xl border border-gray-100 shadow-sm hover:shadow-lg hover:border-red-300 hover:ring-2 hover:ring-red-200 transition-all overflow-hidden">
              ...
              <div className="p-4">
                <div className="flex items-start gap-3 mb-2">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-base font-bold text-gray-800 leading-tight group-hover:text-red-600 transition-colors">{article.title}</p>
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <span className="text-xs ml-auto text-red-500 opacity-0 group-hover:opacity-100 transition-opacity">{locale === 'zh' ? '开始 →' : 'Start →'}</span>
                </div>
              </div>
            </button>
          ))}
